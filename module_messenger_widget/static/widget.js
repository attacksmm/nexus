(function () {
  ...
  function gcLessonGroups(rows) {
    ...
    (Array.isArray(rows)?rows:[]).forEach(function(row){var label=gcLessonLabel(row),kind=/^Созвон\s+\d+$/i.test(label)?"calls":/^(?:Доб\.?(?:авим|авили)?\s+в\s+купивших|Чат)$/i.test(label)?"service":"homework",key=kind+":"+label.toLocaleLowerCase("ru-RU");if(seen[key]){seen[key].value=Boolean(seen[key].value||row.value);return}var item=Object.assign({},row,{label:label,value:Boolean(row.value)});seen[key]=item;groups[kind].push(item)});
    return groups;
  }
  ...
  async function loadGetCourseLessons(card) { try { var data=await gcLessonsRequest(card.item);card.item.lessons=data.lessons||[];card._lessonsError=""; } catch(error) { card._lessonsError=error.message||"Не удалось загрузить ДЗ и созвоны"; } finally { card._lessonsLoading=false;if(getcourseMiniOpen)showGetCourseMini(card); } }
  ...
  function showGetCourseMini(card) {
    ...
    var startParts=!card._partsStarted;if(startParts){card._partsStarted=true;card._lessonsLoading=true;card._accessLoading=true;card._lessonsError="";card._accessError="";}
    ...
    var actions=gcNode("div","gc-actions"),refresh=gcNode("button","tool","Обновить"),apply=gcNode("button","tool","Проверить и применить");refresh.type=apply.type="button";refresh.disabled=Boolean(access.pending);apply.disabled=!Object.keys(draft).length||Boolean(access.pending);refresh.onclick=function(){card._accessLoading=true;showGetCourseMini(card);loadGetCourseAccess(card)};apply.onclick=async function(){busy(apply,"Проверяем и ставим в очередь…");try{var changes=Object.entries(draft).map(function(pair){return{group_id:pair[0],enabled:pair[1]}}),preview=await gcAccessRequest(item,{action:"preview",changes:changes}),result=await gcAccessRequest(item,{action:"apply",request_id:preview.request_id});card.access=result.access||preview.access||card.access;card._draft={};card._accessError="";showGetCourseMini(card)}catch(error){card._accessError=error.message||"Не удалось поставить изменения в очередь";showGetCourseMini(card)}};actions.append(refresh,apply);pane.append(actions);if(startParts){loadGetCourseLessons(card);loadGetCourseAccess(card)}
  }
