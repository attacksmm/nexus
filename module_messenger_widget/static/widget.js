(function () {
  ...
  function showGetCourseMini(card) {
    ...
    var lessons=gcLessonGroups(item.lessons),lessonSections=[["ДЗ · только просмотр",lessons.homework],["Созвоны · только просмотр",lessons.calls],["Этапы · только просмотр",lessons.service]];lessonSections.forEach(function(pair){if(pair[1].length)pane.append(gcLessonSection(pair[0],pair[1]))});if(!lessonSections.some(function(pair){return pair[1].length}))pane.append(gcLessonSection("ДЗ · только просмотр",[]));
    ...
    if(!access.ok){var ready=gcAccessTime(access.next_check_at||access.ready_by),failure=gcNode("div","gc-notice",access.refresh_queued?"GetCourse сейчас занят. Следующая автоматическая попытка — "+ready+". Если GetCourse ещё не ответит, Nexus продолжит проверять сам.":gcFriendlyAccessError(access.error)),retry=gcNode("button","tool","Проверить сейчас");retry.type="button";retry.onclick=function(){loadLive(retry)};failure.append(document.createElement("br"),document.createElement("br"),retry);pane.append(failure);return}
    if(access.pending) pane.append(gcNode("div","gc-pending","Запрос принят. Проверка результата запланирована примерно на "+gcAccessTime(access.next_check_at||access.ready_by)+". Окно можно закрыть — Nexus продолжит сам."));
