(function () {
  ...
  function showGetCourseMini(card) {
    ...
    var lessons=Array.isArray(item.lessons)?item.lessons:[],section=gcNode("section","gc-section"),title=gcNode("h3","","ДЗ · только просмотр · выполнено "+lessons.filter(function(row){return row.value}).length+" из "+lessons.length),lessonList=gcNode("div","gc-chip-list");lessons.forEach(function(row){lessonList.append(gcNode("span","gc-chip"+(row.value?" on":""),(row.value?"✓ ":"○ ")+gcLessonLabel(row)))});section.append(title,lessonList);pane.append(section);
    ...
    if(!access.ok){var ready=gcAccessTime(access.ready_by||access.next_check_at),failure=gcNode("div","gc-notice",access.refresh_queued?"Доступы обновляются автоматически. Откройте карточку после "+ready+" — данные уже будут готовы.":gcFriendlyAccessError(access.error)),retry=gcNode("button","tool","Проверить сейчас");retry.type="button";retry.onclick=function(){loadLive(retry)};failure.append(document.createElement("br"),document.createElement("br"),retry);pane.append(failure);return}
    if(access.pending) pane.append(gcNode("div","gc-pending","Запрос принят. Изменения будут выполнены примерно до "+gcAccessTime(access.ready_by||access.next_check_at)+". Окно можно закрыть."));
