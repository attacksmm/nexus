...
class GetCourseWazzupLogicTests(unittest.TestCase):
    ...
    def test_amocrm_widget_has_one_size_setting_and_valid_manifest_settings(self):
        ...
        self.assertIn("if(!contextReady){context=next;return}const snapshot=readCardSnapshot()", amo)
        self.assertIn("if(!warmResume)enrichCardContext(next)", amo)
        ...
        self.assertIn("while(expectedGeneration===bootGeneration&&token)", amo)
        ...
        self.assertEqual(manifest["widget"]["version"], "1.9.4")
