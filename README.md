...
Общий виджет Wazzup, VK и Telegram Personal для GetCourse и amoCRM. Диалоги и шаблоны хранятся в собственной SQLite, а связанные аккаунты и переменные читаются через компактный дисковый индекс `customer-db`.

- панель: `/nexus/#messenger-widget`;
- GetCourse: `/nexus/messenger-widget/static/widget.js`;
- amoCRM: `/nexus/messenger-widget/static/downloads/widget.zip`;
- общие и личные шаблоны с `{{contact.name}}`, UTM и полями связанных аккаунтов;
- настраиваемые идентификаторы полей сделки, контакта, заказа и пользователя;
- прежний `/nexus/getcourse-wazzup/api|static` остаётся совместимым alias.
...
Файловое хранилище для Nexus. Поддерживает папки, создание текстовых файлов, возобновляемую загрузку больших файлов и прямые публичные ссылки на файлы.
...
- типы файлов не ограничиваются приложением;
- потенциально исполняемые в браузере типы отдаются как скачивание, а не inline-просмотр;
- лимит размера в приложении не задан, фактический предел определяется диском, nginx/proxy и соединением.
...
Командный модуль рассылок VK и Telegram. Читает только разрешённые администратором листы `customer-db`, хранит шаблоны и вложения в системной папке `file-storage`, а кампании, получателей, попытки и внешние ID сообщений — в собственной SQLite.

Ключевые возможности:

- фильтры AND/OR по системным и вложенным JSON-полям;
- отдельный редактор сегментов: листы, условия, ручные получатели, принудительное добавление и исключение ID;
- обязательный выбор готового сегмента при создании рассылки и snapshot сегмента при запуске;
- персонализация через `{{field}}`;
- визуальный конструктор универсальной VK/Telegram-клавиатуры с рядами, drag-and-drop кнопок и JSON;
- загрузка вложений в шаблон через `file-storage` и отдельный предпросмотр;
- немедленный и отложенный запуск, пауза, продолжение, отмена и повтор ошибок;
- восстановление очереди после перезапуска без повторной отправки уже успешным получателям;
- удаление отправленных сообщений через API VK и Telegram;
- роли Nexus: viewer для просмотра, editor для работы, admin для настройки листов и удаления истории.

Секреты задаются только через ENV Nexus:

- `SBKVD_LETTER_VK_TOKEN`;
...
- режимы контекста: `0` не читает и не пишет историю, `1` читает краткий контекст без записи, `2` читает краткий контекст и сохраняет вопрос/ответ, `3` читает полный контекст и сохраняет, `4` читает сводку и последние реплики, сохраняет вопрос/ответ и автоматически обновляет краткую сводку;
...
- генерация и сохранение краткой сводки-рассказа о клиенте: кто клиент, что известно о собаке, какая проблема и что уже обсуждали.
...
Для Senler можно отправлять `context: 4`: endpoint вернет ответ по краткой сводке и последним четырём парам реплик, сохранит вопрос/ответ и обновит сводку в фоне.
...
Полноэкранный чат для команды Sobakovod поверх модуля `openrouter`. Модуль использует отдельные GPT-аккаунты с входом только по логину, показывает каждому человеку только выданные ему prompt-файлы и модели, а историю бесед хранит в своей SQLite-БД.

Основной адрес:

```text
/nexus/sbkvd-gpt/panel/      # управление доступами в Nexus
/nexus/sbkvd-gpt/panel/chat/ # отдельный полноэкранный чат
```

Ключевые правила:

- промпты берутся из `file-storage` и не копируются в модуль;
- выбранная в интерфейсе модель передаётся в OpenRouter явно и имеет приоритет над дефолтами OpenRouter;
- при включённом «Хранить контекст» история сохраняется только в `sbkvd-gpt`;
- при выключенном контексте запрос не сохраняется;
- управление GPT-аккаунтами, доступными промптами, моделями и названиями промптов доступно только Nexus-пользователям с ролью `admin`;
- для генерации должен быть активен модуль `openrouter` с настроенным `OPENROUTER_API_KEY`.
