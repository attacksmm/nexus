...
class MobileShellTests(unittest.TestCase):
    ...
    def test_module_panel_bridge_is_central_and_idempotent(self):
        ...
        self.assertIn('filter:invert(.18)', injected)
        self.assertIn('filter:none', injected)
        self.assertIn('filter:invert(1) hue-rotate(180deg)', injected)
        self.assertIn('body{background:#060708!important}', injected)
        self.assertIn('body{background:#03060d!important}', injected)
        self.assertIn('body{background:#0c0a08!important}', injected)
        self.assertIn('--bg:#060708!important;--panel:#060708!important', injected)
        self.assertIn('--bg:#03060d!important;--panel:#03060d!important', injected)
        self.assertIn('--bg:#0c0a08!important;--panel:#0c0a08!important', injected)
