...
DIRECT_ABUSE_RE = re.compile(
    r"(?i)(?:"
    r"\b(?:ты|вы|тебя|тебе|вам|вас|админ\w*|модератор\w*|куратор\w*|команд\w*|кинолог\w*|участник\w*|люди\s+(?:тут|здесь|в\s+чат[еау]))\b"
    ...
    r".{0,80}\b(?:ты|вы|админ\w*|модератор\w*|куратор\w*|команд\w*|кинолог\w*|участник\w*|чат)\b"
    r")"
)
...
VK_SYSTEM_PROMPT = """Ты модератор клубного чата ВК. Твоя задача — определить, является ли сообщение:
- негативом (оскорбления, агрессия, токсичность, угрозы),
- скамом (реклама сторонних услуг, ссылки на подозрительные ресурсы, мошеннические предложения, "заработать деньги", казино, ставки, крипто-схемы, продажа аккаунтов, попытки увести людей в другие чаты).

Всё обычное общение про собак, обучение, визитки, вопросы участников и бытовые обсуждения — это "ок".
Важное правило по ссылкам: wildberries, ozon, обычные ссылки yandex и mail.ru, домены sobakovod.pro и обычные ссылки на товары/файлы не считать негативом сами по себе. Если сомневаешься, выбирай "ок".

Ответь ТОЛЬКО одним словом из списка: негатив, скам, ок.
...
DEFAULT_SETTINGS = {
    ...
    "tg_welcome_enabled": "false",
    "vk_welcome_enabled": "false",
    "history_retention_days": "90",
    "tg_allowed_adders": TG_ALLOWED_ADDERS_DEFAULT,
    "telegram_log_chat_id": "-1002852064172",
    "telegram_chat_allowlist": "",
    "chat_title_contains": "Чат клуба \"Современный собаковод\";Чат клуба «Современный собаковод»",
    "vk_allowed_admins": VK_ALLOWED_ADMINS_DEFAULT,
    "vk_trusted_senders": VK_TRUSTED_SENDERS_DEFAULT,
    "vk_training_title_allowlist": "Чат клуба «Современный собаковод»",
    "vk_ai_prompt_path": "prompts/moderator_prompt.txt",
...
async def setup(ctx):
    ...
    _log(
        "info",
        "chat-moderator setup completed; runtime is not auto-started; corrected_stale=%s",
...
def _env_default(key: str, fallback: str) -> str:
    env_key = {
        "runtime_enabled": "NEXUS_CHAT_MODERATOR_RUNTIME_ENABLED",
        "tg_enabled": "NEXUS_CHAT_MODERATOR_TG_ENABLED",
        "vk_enabled": "NEXUS_CHAT_MODERATOR_VK_ENABLED",
        "dry_run": "NEXUS_CHAT_MODERATOR_DRY_RUN",
        "tg_send_responses": "NEXUS_CHAT_MODERATOR_TG_SEND_RESPONSES",
        "vk_send_responses": "NEXUS_CHAT_MODERATOR_VK_SEND_RESPONSES",
        "tg_welcome_enabled": "NEXUS_CHAT_MODERATOR_TG_WELCOME_ENABLED",
        "vk_welcome_enabled": "NEXUS_CHAT_MODERATOR_VK_WELCOME_ENABLED",
        "telegram_log_chat_id": "TELEGRAM_LOG_CHAT_ID",
        "telegram_chat_allowlist": "NEXUS_CHAT_MODERATOR_TG_CHAT_ALLOWLIST",
...
def _is_club_chat_title(title: str) -> bool:
    ...
    value = _norm_text(title)
    return "чат клуба" in value and "современный собаковод" in value


def _telegram_chat_zone(chat: Any) -> str | None:
    chat_id = str(getattr(chat, "id", "") or "").strip()
    allowlist = {item.strip() for item in re.split(r"[,\\s]+", str(_settings().get("telegram_chat_allowlist") or "")) if item.strip()}
    if chat_id in allowlist or _telegram_peer_id_from_chat_id(chat_id) in allowlist:
        return "club"
    title = getattr(chat, "title", "") or ""
    if _is_club_chat_title(title):
        return "club"
    return None
...
class TelegramModeratorRuntime:
    ...
    async def start(self, settings: dict[str, str]) -> None:
        ...
        async def handle_message(update: Any, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            send_responses = _truthy(settings.get("tg_send_responses"))
            if dry_run:
                _record_action(
                    platform="telegram",
                    action="would_delete",
                ...
                if reply_template_key and send_responses:
        ...
        async def handle_new_members(update: Any, context: ContextTypes.DEFAULT_TYPE) -> None:
            ...
            if not chat_zone:
                _log(
                    "info",
                    "chat-moderator ignored telegram new_member event: chat_id=%s title=%r reason=title_not_matched",
                    chat_id,
...
class VKModeratorRuntime:
    ...
    def get_chat_zone(self, peer_id: int) -> str | None:
        ...
        if _is_club_chat_title(title) or (not _configured_title_fragments(self.settings) and title in allowlist):
            return "club"
        return None
    ...
    async def process_chat_update(self, event: Any) -> None:
        ...
        if zone != "club":
            if zone != "logs":
                _log(
                    "info",
                    "chat-moderator ignored vk chat update: peer_id=%s title=%r reason=title_not_matched",
                    peer_id,
                    title,
                )
            return
...
async def list_actions(
    ...
    where: list[str] = [
        """(
            ...
            OR (platform='vk' AND (peer_id='' OR zone='club'))
        )"""
    ]
    params: list[Any] = []
    if platform:
