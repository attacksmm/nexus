...
from module_chat_moderator import router as module


def _ready(tmp_path, monkeypatch):
    db_path = tmp_path / "chat-moderator.db"
...
def test_staff_connector_updates_exact_vk_id_and_deactivation_removes_it(tmp_path, monkeypatch):
    ...
    replay = module.service_staff_apply(
        employee=employee,
        config={"allowed_admin": True, "trusted_sender": False},
        operation="upsert",
        idempotency_key="replay-does-not-matter",
    )
    ...
    exported = next(item for item in module.service_staff_list() if item["local_id"] == "303")
    assert exported["identities"] == [{"provider": "vk", "external_id": "303"}]
...
def test_shared_course_registry_remains_unmanaged_and_effective(tmp_path, monkeypatch):
    ...
    result = module.service_staff_apply(
        employee=_employee(909), config={}, operation="deactivate",
    )
    ...
    assert capability["managed"] is False
    assert capability["read_only"] is True
    assert capability["effects"] == ["auto_promote_vk_chat_admin", "protect_from_moderation"]
