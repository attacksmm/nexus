...
def _normalize_order(payload: dict[str, Any], settings: dict[str, str], forced_payment_state: str = "") -> dict[str, Any]:
    ...
    fields = {
        ...
        "positions": positions,
        "offers": offers,
        "tags": _jsonish(payload.get("tags", "")),
        "order_tags": _jsonish(payload.get("order_tags", "")),
        "tag_names": _jsonish(payload.get("tag_names", "")),
        "autopayment": _clean(payload.get("autopayment") or payload.get("is_autopayment"), 40),
        "title": title,
        ...
        "order_utm_source": _clean(payload.get("orderUtmS") or payload.get("order_utm_source"), 500),
        "order_utm_medium": _clean(
            payload.get("orderUtmM") or payload.get("order_utm_medium") or payload.get("order_medium"),
            500,
        ),
        "order_utm_campaign": _clean(payload.get("orderUtmCa") or payload.get("order_utm_campaign"), 500),
        "order_utm_content": _clean(payload.get("orderUtmCo") or payload.get("order_utm_content"), 500),
        "order_utm_term": _clean(payload.get("orderUtmT") or payload.get("order_utm_term"), 500),
        ...
        "date_add": _clean(payload.get("date_add"), 100),
