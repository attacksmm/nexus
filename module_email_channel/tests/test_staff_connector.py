...
def test_email_upsert_requires_explicit_messenger_link(ready):
    employee = {
        "id": "employee-1", "display_name": "Анна", "status": "active",
        "identities": [], "source_links": {},
    }
...
def test_email_profile_follows_messenger_id_and_deactivation_preserves_history(ready):
    async def run():
        employee = {
            ...
            "source_links": {"messenger-widget": {"local_id": "42"}},
        }
        ...
        try:
            ...
            thread = await db.execute(
                """INSERT INTO email_threads(public_token,mailbox_id,client_email,created_at,updated_at)
                   VALUES(?,?,?,?,?)""",
