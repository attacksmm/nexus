...
TERMINAL_FAILURE_EVENTS = {"hard", "bounced", "dropped", "spam", "complained"}
...
def _event_status(event: str) -> tuple[str, bool, bool]:
    ...
    if value in TERMINAL_FAILURE_EVENTS: return "failed", False, True
...
async def _store_provider_event(fields: dict[str, str]) -> bool:
    ...
    status, opened, suppress = _event_status(event)
    db = await _connect()
    try:
        cursor = await db.execute(
        ...
        if inserted and suppress and address:
            await db.execute(
                ...
                (address, event, event_hash, now, now),
            )
            await _cancel_queued_for_suppression(db, address, event, now)
        await db.commit()
        return inserted
    finally:
        await db.close()
